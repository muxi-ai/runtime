...
**Important Notes:**
1. **Development Database**: Use `postgresql://muxi@localhost/muxi_test` for testing migrations on PostgreSQL
2. **Final Migration Strategy**: After validating migrations work, update `migrations/init_schema.sql` and `migrations/init_schema_sqlite.sql` instead of keeping separate migration files (new installs should get schema directly)
3. **SQLite Compatibility**: Everything must work on both PostgreSQL and SQLite, even though SQLite is single-user mode (user_id always "0"). SQLite should not error out on multi-identity features.

**Files:**
- `migrations/init_schema.sql` (SINGLE SOURCE OF TRUTH - PostgreSQL)
- `migrations/init_schema_sqlite.sql` (SINGLE SOURCE OF TRUTH - SQLite)
...
1. **Schema Update Workflow:**
   - Update both `init_schema.sql` and `init_schema_sqlite.sql` directly
   - Test on PostgreSQL test database: `postgresql://muxi@localhost/muxi_test`
   - Verify SQLite compatibility
   - No incremental migrations - init schemas are SINGLE SOURCE OF TRUTH
...
- All E2E tests must be created in `e2e/tests/17_multiple_identities/` directory
- Tests must validate both PostgreSQL (`postgresql://muxi@localhost/muxi_test`) and SQLite backends
- SQLite tests must verify single-user mode (user_id="0") doesn't error with multi-identity features
...
### New Files
- `src/muxi/utils/user_resolution.py` (~350 lines - user resolution utilities)
- `docs/features/multi-identity.md` (~665 lines - comprehensive documentation)
- `docs/features/multi-identity-quickstart.md` (~315 lines - quick start guide)
- `docs/features/README.md` (~31 lines - features index)
