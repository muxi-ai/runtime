...
class TaskSpecification(BaseModel):
    ...
    required_capabilities: List[str] = Field(
        ...,
        min_items=1,
        description="Capabilities an agent must have to execute this task"
    )
    ...
    estimated_complexity: float = Field(
        ...
        ge=1.0,
        le=10.0,
        description="Estimated task complexity (1-10 scale)"
    )
    ...
    input_requirements: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Required inputs and their specifications"
    )
    constraints: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Any constraints or limitations for task execution"
    )
...
class TaskExecutionState(BaseModel):
    ...
    last_attempt_time: Optional[datetime] = Field(
        default=None,
        description="Time of the last execution attempt"
    )
    estimated_completion_time: Optional[datetime] = Field(
        default=None,
        description="Estimated time when task will complete"
    )

    # Additional runtime state
    is_critical_path: Optional[bool] = Field(
        default=None,
        description="Whether this task is on the critical execution path"
    )
    ...
    retry_after: Optional[datetime] = Field(
        default=None,
        description="Earliest time to retry if in retry state"
    )
    ...
    def can_start(self) -> bool:
        ...
        return (
            ...
            len(self.blocked_by) == 0 and
            (self.retry_after is None or datetime.now() >= self.retry_after)
        )
...
class TaskExecutionResult(BaseModel):
    ...
    outputs: Dict[str, Any] = Field(
        ...
        description="Task outputs keyed by output name"
    )
    ...
    attempt_number: int = Field(
        default=1,
        ge=1,
        description="Which attempt this result represents"
    )

    # Additional result information
    raw_response: Optional[str] = Field(
        default=None,
        description="Raw response from the executing agent"
    )
    artifacts: Optional[Dict[str, Any]] = Field(
        default=None,
        description="Any artifacts produced during execution"
    )
