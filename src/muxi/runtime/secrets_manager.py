    ...
    async def initialize_encryption(self) -> None:
        ...
        if OBSERVABILITY_AVAILABLE:
            try:
                ...
                obs_manager.log_event(
                    event_type=EventType.ENCRYPTION_STARTED,
        ...
        try:
            ...
            if OBSERVABILITY_AVAILABLE:
                try:
                    obs_manager.log_event(
                        event_type=EventType.ENCRYPTION_COMPLETED,
                        level=EventLevel.INFO,
    ...
    async def get_secret(self, name: str) -> Optional[Any]:
        ...
        if OBSERVABILITY_AVAILABLE:
            try:
                obs_manager = ObservabilityManager.get_instance()
                obs_manager.log_event(
                    event_type=EventType.SECRET_RETRIEVAL_STARTED,
        ...
        try:
            ...
            async with self._lock:
                ...
                if OBSERVABILITY_AVAILABLE:
                    try:
                        obs_manager.log_event(
                            event_type=EventType.SECRET_RETRIEVAL_COMPLETED,
                            level=EventLevel.DEBUG,
    ...
    async def delete_secret(self, name: str) -> bool:
        ...
        if OBSERVABILITY_AVAILABLE:
            try:
                obs_manager = ObservabilityManager.get_instance()
                obs_manager.log_event(
                    event_type=EventType.SECRET_DELETION_STARTED,
                    level=EventLevel.INFO,
            ...
            async with self._lock:
                ...
                if normalized_name not in secrets:
                    ...
                    if OBSERVABILITY_AVAILABLE:
                        try:
                            obs_manager.log_event(
                                event_type=EventType.SECRET_DELETION_COMPLETED,
                                level=EventLevel.DEBUG,
    ...
    async def interpolate_secrets(self, value: Any) -> Any:
        ...
        if OBSERVABILITY_AVAILABLE:
            try:
                obs_manager = ObservabilityManager.get_instance()
                obs_manager.log_event(
                    event_type=EventType.SECRET_INTERPOLATION_STARTED,
                    level=EventLevel.DEBUG,
        ...
        try:
            ...
            if OBSERVABILITY_AVAILABLE:
                try:
                    obs_manager.log_event(
                        event_type=EventType.SECRET_INTERPOLATION_COMPLETED,
                        level=EventLevel.DEBUG,
    ...
    async def clear_all_secrets(self) -> None:
        ...
        if OBSERVABILITY_AVAILABLE:
            try:
                obs_manager = ObservabilityManager.get_instance()
                obs_manager.log_event(
                    event_type=EventType.SECRET_CLEARING_STARTED,
        ...
        try:
            ...
            async with self._lock:
                ...
                if OBSERVABILITY_AVAILABLE:
                    try:
                        obs_manager.log_event(
                            event_type=EventType.SECRET_CLEARING_COMPLETED,
                            level=EventLevel.WARNING,
    ...
    async def import_secrets(
        ...
        if OBSERVABILITY_AVAILABLE:
            try:
                obs_manager = ObservabilityManager.get_instance()
                obs_manager.log_event(
                    event_type=EventType.SECRET_IMPORT_STARTED,
                    level=EventLevel.INFO,
        ...
        try:
            ...
            if OBSERVABILITY_AVAILABLE:
                try:
                    obs_manager.log_event(
                        event_type=EventType.SECRET_IMPORT_COMPLETED,
    ...
    async def export_secrets(self) -> Dict[str, Any]:
        ...
        if OBSERVABILITY_AVAILABLE:
            try:
                obs_manager = ObservabilityManager.get_instance()
                obs_manager.log_event(
                    event_type=EventType.SECRET_EXPORT_STARTED,
                    level=EventLevel.INFO,
        ...
        try:
            ...
            async with self._lock:
                ...
                if OBSERVABILITY_AVAILABLE:
                    try:
                        obs_manager.log_event(
                            event_type=EventType.SECRET_EXPORT_COMPLETED,
                            level=EventLevel.INFO,
