...
- When a prior step returns a LIST of records (e.g. `list-folder-files`, `search-mail`, `list-calendar-events`), use a predicate filter `{{NAME[key=value].field}}` to pick the specific record you intend BEFORE extracting a field. Without a predicate, `{{NAME.field}}` resolves to the first record the runtime encounters, which is almost never the one you meant.
