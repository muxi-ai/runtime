...
class ResourceManager:
    ...
    async def optimize_allocation(
        self, parallel_groups: List[ParallelGroup], available_agents: List[str]
    ) -> ResourceAllocation:
        """
        ...
        valid_agents = [
            agent_id for agent_id in available_agents if agent_id in self.agent_capabilities
        ]
    ...
    async def _allocate_group_tasks(
        self, group: ParallelGroup, available_agents: List[str], allocation: ResourceAllocation
    ) -> None:
    ...
    async def _score_agents_for_task(
        self, task: TaskNode, available_agents: List[str]
    ...
    async def _calculate_capability_match(self, task: TaskNode, agent: AgentCapability) -> float:
    ...
    async def _calculate_allocation_metrics(
        self, allocation: ResourceAllocation, parallel_groups: List[ParallelGroup]
    ) -> None:
        ...
        if total_sequential_time > 0:
            allocation.parallel_efficiency = min(
                1.0, allocation.total_estimated_time / total_sequential_time
            )
            ...
            if agent_id in self.agent_capabilities:
                ...
                if len(tasks) > agent.max_concurrent_tasks:
                    bottleneck = BottleneckInfo(
                        ...
                        suggested_resolution=f"Redistribute tasks from agent {agent_id} or add more agents",  # noqa: E501
        ...
        for capability, demand in capability_demand.items():
            ...
            if supply < demand:
                bottleneck = BottleneckInfo(
                    ...
                    description=f"Shortage of '{capability}' capability: need {demand}, have {supply}",  # noqa: E501
                    suggested_resolution=f"Add {demand - supply} agents with '{capability}' capability",  # noqa: E501
    ...
    async def rebalance_allocation(
        self, allocation: ResourceAllocation, target_balance_threshold: float = 0.8
    ) -> ResourceAllocation:
        """
        ...
        overloaded = [
            agent_id
            for agent_id, size in workload_sizes.items()
