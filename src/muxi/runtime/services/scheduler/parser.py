...
class ScheduleParser:
    ...
    def __init__(self):
        ...
        self.day_patterns = {
            ...
            'business days': '1-5', 'work days': '1-5'
        }
    ...
    async def parse_schedule(self, schedule_text: str, timezone: str = "UTC") -> Union[str, Dict[str, Any]]:
        ...
        if job_type == "one_time":
            ...
            observability.observe(
                ...
                data={
                    "original_text": schedule_text,
                    "scheduled_for": datetime_result["scheduled_for"].isoformat() if datetime_result else None,
    ...
    async def _detect_job_type(self, schedule_text: str) -> str:
        ...
        return await self._llm_detect_job_type(schedule_text)
...
"""
        ...
        try:
            ...
            scheduled_datetime = target_tz.localize(datetime(
    ...
    def _fallback_parse_datetime(self, schedule_text: str, timezone: str) -> Dict[str, Any]:
        ...
        if "tomorrow" in schedule_lower:
            ...
            if current_time.month == 12:
                scheduled_time = current_time.replace(year=current_time.year + 1, month=1, day=1, hour=9, minute=0, second=0, microsecond=0)
            else:
                scheduled_time = current_time.replace(month=current_time.month + 1, day=1, hour=9, minute=0, second=0, microsecond=0)
        else:
...
"""
    ...
    async def generate_exclusion_rules(self, exclusion_descriptions: List[str]) -> List[Dict[str, Any]]:
        """
    ...
    async def _generate_single_exclusion_rule(self, llm: LLM, description: str) -> Optional[Dict[str, Any]]:
        """
...
Examples:
- "except weekends" → {{"type": "cron", "pattern": "* * * * 0,6", "description": "Exclude weekends (Saturday and Sunday)"}}
...
- "except the last Friday of each month" → {{"type": "complex_date", "pattern": "last_friday_of_month", "description": "Exclude the last Friday of each month"}}
- "except the first Monday of the month" → {{"type": "complex_date", "pattern": "first_monday_of_month", "description": "Exclude the first Monday of each month"}}
- "except every 3rd Tuesday" → {{"type": "complex_date", "pattern": "nth_weekday:3:tuesday", "description": "Exclude every 3rd Tuesday of the month"}}
- "außer am letzten Freitag des Monats" → {{"type": "complex_date", "pattern": "last_friday_of_month", "description": "Exclude the last Friday of each month"}}
- "sauf le premier lundi du mois" → {{"type": "complex_date", "pattern": "first_monday_of_month", "description": "Exclude the first Monday of each month"}}
