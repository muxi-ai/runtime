...
class MultiLLMCircuitBreaker:
    """
    Manages multiple circuit breakers for different LLM providers.

    This allows the scheduler to fall back to alternative LLM providers
    when one is experiencing issues.
    """

    def __init__(self, default_config: Optional[dict] = None):
        """
        Initialize multi-provider circuit breaker.
    ...
    def get_breaker(self, provider: str) -> LLMCircuitBreaker:
        ...
        if provider not in self.breakers:
            self.breakers[provider] = LLMCircuitBreaker(**self.default_config)
    ...
    def get_available_providers(self, providers: list[str]) -> list[str]:
        ...
        Returns:
            List of providers with closed or half-open circuits
        """
        return [provider for provider in providers if self.get_breaker(provider).is_available()]

    def get_all_stats(self) -> dict[str, dict]:
        """Get statistics for all circuit breakers."""
        return {provider: breaker.get_stats() for provider, breaker in self.breakers.items()}
