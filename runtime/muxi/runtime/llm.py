...
# in the Muxi framework through the onellm package. It defines:
#
# 1. LLM Class
#    - Concrete implementation using onellm package
...
#
# The LLM class uses the onellm package to provide a consistent interface
# across all supported language model providers.
# =============================================================================

from typing import Any, Dict, List, Optional, Union

from onellm import ChatCompletion, Embedding  # type: ignore
from onellm.config import set_api_key as onellm_set_api_key  # type: ignore
from loguru import logger


def set_llm_api_key(api_key: str, provider: str) -> None:
    """
    Set the API key for a specific provider in onellm.

    This is a convenience function that wraps onellm.config.set_api_key.
    ...
    """
    onellm_set_api_key(api_key, provider)
    logger.debug(f"API key set for provider: {provider}")


class LLM:
    """
    Unified model implementation using onellm.

    This class provides a standardized interface for all language model providers
    using the onellm package, which supports multiple providers through a
    consistent API.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initialize a model using onellm.
        ...
        Args:
            ...
            api_key: API key for the provider. If provided, it will be set using
                onellm.config.set_api_key for the appropriate provider.
    ...
    async def chat(
        ...
        **kwargs: Any,
        ...
        """
        Generate a chat completion using onellm.
