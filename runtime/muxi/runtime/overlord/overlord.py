...
"""
    ...
    async def chat(
        ...
        """
        Chat with the overlord using enhanced workflow orchestration.

        This is the primary interface for intelligent message processing, automatically
        determining whether to handle requests through simple agent routing or complex
        multi-agent workflows.

        Main entry point that analyzes requests and routes to either:
        1. Traditional agent routing (simple requests)
        2. Workflow orchestration (complex requests)
        3. Plan approval workflow (when user requests plan preview)

        Args:
            user_message: The user's message or request to process
            user_id: Optional user identifier for multi-user support
            conversation_id: Optional conversation identifier for grouping messages
            context: Optional conversation context
            enable_workflow: Override default workflow behavior (True/False/None)

        Returns:
            Response string from agent or workflow system, maintaining overlord persona

        Example:
            >>> response = await overlord.chat("Hello, how are you?")
            >>> # Simple request - direct agent routing

            >>> response = await overlord.chat("Research AI trends and write a report")
            >>> # Complex request - automatic workflow orchestration

            >>> response = await overlord.chat("Refactor my code. Show me your plan first.")
        ...
        try:
            ...
            analysis = await self.request_analyzer.analyze_request(user_message, context)
            ...
            # Graceful fallback to base overlord
            return await self._handle_simple_request(
                user_message, user_id, conversation_id, context
            )

    def _should_use_workflow(
        self, analysis: RequestAnalysis, enable_workflow: Optional[bool]
    ) -> bool:
        """
        Determine if request should use workflow orchestration.

        Args:
            analysis: Request analysis results
            enable_workflow: User override

        Returns:
            True if workflow mode should be used
        """
        # User explicit override
        if enable_workflow is not None:
            return enable_workflow

        # User explicitly requests plan preview
        if analysis.requires_approval:
            return True

        # Analysis suggests decomposition is beneficial
        if analysis.requires_decomposition:
            return True

        # Default behavior
        return self.enable_workflow_by_default
    ...
    async def _handle_workflow_request(
        ...
        """
        Handle request using workflow orchestration.
        ...
        try:
            ...
            # If user approval is required, present plan
            if analysis.requires_approval:
                return await self._handle_plan_approval(workflow, user_id, conversation_id)

            # Execute workflow directly
            return await self._execute_workflow(workflow, user_id, conversation_id, context)
    ...
    async def _handle_plan_approval(
        ...
        Args:
            workflow: Workflow awaiting approval
            user_id: Optional user identifier
            conversation_id: Optional conversation identifier

        Returns:
            Plan preview for user approval
        """
        # Store workflow for approval tracking
        approval_key = self._get_approval_key(user_id, conversation_id)
        self.pending_approvals[approval_key] = workflow

        # Present plan for approval
        plan_preview = await self.approval_manager.present_plan_for_approval(workflow)

        logger.info(f"Presented plan for approval: {workflow.id}")
        return plan_preview

    async def _handle_pending_approvals(
        self,
        user_message: str,
        user_id: Optional[int] = None,
        conversation_id: Optional[str] = None,
        ...
        """
        Check for and handle pending approval responses.
        ...
        Returns:
            Response if this was an approval response, None otherwise
        """
        approval_key = self._get_approval_key(user_id, conversation_id)
        ...
        workflow = self.pending_approvals[approval_key]
        ...
        if approval_status == ApprovalStatus.APPROVED:
            ...
            # Extract user information from approval interaction
            if self.auto_extract_user_info and user_id and user_id != 0:
                await self.handle_user_information_extraction(
                    user_message=user_message,
                    agent_response="Plan approved. Proceeding with execution.",
                    user_id=user_id,
                    agent_id="plan_approval_manager",
                )

            logger.info(f"Plan approved, executing workflow: {workflow.id}")
            return await self._execute_workflow(workflow, user_id, conversation_id)

        elif approval_status == ApprovalStatus.REJECTED:
            # Remove from pending
            del self.pending_approvals[approval_key]

            logger.info(f"Plan rejected for workflow: {workflow.id}")
            return "I understand. Let me know how I can help you differently."

        elif approval_status == ApprovalStatus.MODIFIED:
            # Modify workflow based on feedback
            if instructions:
                try:
                    modified_workflow = await self.task_decomposer.modify_workflow(
                        workflow, instructions
                    )

                    # Update pending workflow
                    self.pending_approvals[approval_key] = modified_workflow

                    # Present updated plan
                    updated_plan = await self.approval_manager.present_plan_for_approval(
                        modified_workflow
                    )

                    logger.info(f"Plan modified for workflow: {workflow.id}")
                    return updated_plan

                except Exception as e:
                    logger.error(f"Error modifying workflow: {e}")
                    return "I had trouble updating the plan. Could you clarify what you'd like me to change?"

        # Still awaiting approval or need clarification
        return instructions
    ...
    async def _execute_workflow(
        ...
        try:
            ...
            if self.auto_extract_user_info and user_id and user_id != 0:
                # Use the original user message and final workflow response for extraction
                original_request = workflow.user_request if hasattr(workflow, 'user_request') else "Complex workflow request"
                await self.handle_user_information_extraction(
    ...
    async def _generate_workflow_response(self, workflow: Workflow) -> str:
        ...
        if workflow.status == WorkflowStatus.COMPLETED:
            ...
            failed_tasks = [
                task for task in workflow.tasks.values() if task.status.value == "failed"
            ]

            if failed_tasks:
                return f"I encountered issues completing your request. {len(failed_tasks)} task(s) failed during execution."
            else:
                return "I was unable to complete your request due to an unexpected error."
    ...
    async def _handle_simple_request(
        ...
        """
        Handle simple request using traditional agent routing.
        ...
        Returns:
            Response from traditional agent routing
        """
        try:
            # Use traditional agent selection and routing
            agent_id = await self.select_agent_for_message(user_message)
            agent = self.get_agent(agent_id)

            # Process message directly with agent
            response = await agent.process_message(user_message, user_id=user_id)

            # Handle extraction if enabled and user is not anonymous
            if self.auto_extract_user_info and user_id and user_id != 0:
                await self.handle_user_information_extraction(
                    user_message=user_message,
                    agent_response=response,
            ...
            logger.error(f"Error in simple request handling: {e}")
            return "I encountered an error processing your request. Please try again."

    def _get_approval_key(self, user_id: Optional[int], conversation_id: Optional[str]) -> str:
        """
        Generate key for tracking pending approvals.
        ...
        Returns:
            Approval tracking key
        """
        return f"{user_id or 'unknown'}:{conversation_id or 'default'}"
    ...
    def get_pending_approvals(self) -> Dict[str, Workflow]:
    ...
    def get_workflow_progress(self, workflow_id: str) -> Optional[Dict[str, Any]]:
        ...
        return self.progress_tracker.get_progress(workflow_id)

    async def cancel_workflow(self, workflow_id: str) -> bool:
        """Cancel an active workflow."""
        success = await self.workflow_executor.cancel_workflow(workflow_id)
        if success and workflow_id in self.active_workflows:
            del self.active_workflows[workflow_id]
        return success

    def add_workflow_agent(self, agent_id: str, agent: Agent):
        """Add an agent to the workflow system."""
        self.add_agent(agent)
        # Update workflow executor with new agent registry
        self.workflow_executor = WorkflowExecutor(agent_registry=self.agents)

    def remove_workflow_agent(self, agent_id: str):
        """Remove an agent from the workflow system."""
        if self.remove_agent(agent_id):
            # Update workflow executor with updated agent registry
            self.workflow_executor = WorkflowExecutor(agent_registry=self.agents)

    def configure_complexity_threshold(self, threshold: float):
        """Configure the complexity threshold for workflow triggering."""
        self.complexity_threshold = threshold
        self.request_analyzer.complexity_threshold = threshold

    def enable_workflow_mode(self, enabled: bool = True):
        """Enable or disable workflow mode by default."""
        self.enable_workflow_by_default = enabled

    def get_system_status(self) -> Dict[str, Any]:
        """Get comprehensive system status including workflow information."""
        status = {
            "agents": len(self.agents),
            "active_workflows": len(self.active_workflows),
            "pending_approvals": len(self.pending_approvals),
            "workflow_enabled": self.enable_workflow_by_default,
            "complexity_threshold": self.complexity_threshold,
            "memory_enabled": {
                "buffer": self.buffer_memory is not None,
