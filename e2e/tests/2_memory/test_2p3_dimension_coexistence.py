...
Verifies:
1. Both memories_768 (Nomic v1.5 default) and memories_1536 (OpenAI)
   tables can coexist on the same PostgreSQL database.
2. Each formation reads/writes only its own dimension table.
3. No cross-contamination between dimension tables.

Post embedding-platform migration (2026-04-22), the default local
embedding model is ``local/nomic-ai/nomic-embed-text-v1.5`` (768-dim,
8k context, Apache-2.0) — was previously 384-dim MiniLM.
"""
...
class TestDimensionCoexistence(BaseE2ETest):
    ...
    async def test_coexistence(self):
        ...
        try:
            ...
            # Phase 1: Load with the default local model (Nomic v1.5, 768-dim)
            # and store a memory.
            print("\n  Phase 1: Formation with default local 768-dim embeddings")
            await self.setup_formation(
                formation_path=formation_dir / "formation-local-384.yaml",
            )

            ltm_768 = self.overlord.long_term_memory
            inner_768 = getattr(ltm_768, "long_term_memory", ltm_768)

            # The dimension probe is lazy — it only fires after the first
            # add(). Write a memory first, then assert the resolved dim.
            user_id = "coexistence_test_user"
            mem_768_id = await inner_768.add(
                content="I work at a tech company in San Francisco",
                metadata={"source": "768_test"},
                external_user_id=user_id,
            )
            print(f"    Created 768-dim memory: {mem_768_id}")
            assert inner_768.dimension == 768, f"Expected 768, got {inner_768.dimension}"
            assert inner_768.MemoryModel.__tablename__ == "memories_768"
            checks_passed.append("768-dim formation loaded correctly")
            checks_passed.append("Memory stored in memories_768")
            ...
            # Phase 3: Verify isolation — 1536 search should NOT find 768 memory
            ...
            has_768_content = any("San Francisco" in t for t in texts_1536)
            # The 768-dim memory should NOT appear in 1536 search (different table)
            if not has_768_content:
                checks_passed.append("No cross-contamination: 768 memory not in 1536 results")
                print("    768-dim memory correctly absent from 1536 search results")
            else:
                # This would mean tables aren't properly isolated
                print("    WARNING: 768-dim content found in 1536 results (unexpected)")
