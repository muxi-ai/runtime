...
def fix_file(filepath):
    ...
    pattern2 = r'(\s+)# Collect streaming response\s+response = ""\s+async for chunk in (response_gen|response):\s+response \+= chunk'  # noqa: E501
    ...
    def replace2(match):
        ...
        return f"{indent}# Extract response text\n{indent}response = {obj_name}.content if hasattr({obj_name}, 'content') else str({obj_name})"  # noqa: E501
