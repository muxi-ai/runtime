#!/usr/bin/env python3
"""Test test_6_4: Advanced Knowledge Search"""
...
class Testtest64(BaseKnowledgeTest):
    """Test class for test_6_4."""

    async def test_main(self):
        """Test advanced knowledge search functionality."""
        test_name = "test_6_4"
        self.print_test_header(test_name, "Advanced Knowledge Search")
        ...
        try:
            ...
            # Test 1: Semantic Knowledge Search
            print("\n  1. Testing Semantic knowledge search...")
            prompt1 = "How can I automate my deployment process?"
            ...
            if result1 and len(result1) > 10:
                print(f"    ✓ Knowledge retrieved for Semantic knowledge search ({len(result1)} chars)")
                checks_passed.append("Semantic search working")

                # Verification logic
                deploy_keywords = ['deploy', 'automation', 'process']
keywords_found = [kw for kw in deploy_keywords if kw.lower() in result1.lower()]
                ...
                print("    ✗ No meaningful knowledge retrieved for Semantic knowledge search")
                all_passed = False

            # Test 2: Complex Query Processing
            print("\n  2. Testing Complex query processing...")
            prompt2 = "What's the difference between MUXI tiers and how do I choose?"
            ...
            if result2 and len(result2) > 10:
                print(f"    ✓ Knowledge retrieved for Complex query processing ({len(result2)} chars)")
                checks_passed.append("Complex query processed")

                # Verification logic
                assert 'tier' in result2.lower() or 'choose' in result2.lower()

                if 'keywords_found' in locals() and keywords_found:
                    print(f"    ✓ Found relevant keywords: {', '.join(keywords_found)}")
                    checks_passed.append(f"Keywords found: {', '.join(keywords_found)}")
            else:
                print("    ✗ No meaningful knowledge retrieved for Complex query processing")
                all_passed = False

            # Test 3: Knowledge Synthesis
            print("\n  3. Testing Knowledge synthesis...")
            prompt3 = "Help me understand the complete MUXI ecosystem"
            ...
            if result3 and len(result3) > 10:
                print(f"    ✓ Knowledge retrieved for Knowledge synthesis ({len(result3)} chars)")
                checks_passed.append("Knowledge synthesis working")

                # Verification logic
                assert len(result3) > 100  # Should be comprehensive

                if 'keywords_found' in locals() and keywords_found:
                    print(f"    ✓ Found relevant keywords: {', '.join(keywords_found)}")
                    checks_passed.append(f"Keywords found: {', '.join(keywords_found)}")
            else:
                print("    ✗ No meaningful knowledge retrieved for Knowledge synthesis")
                all_passed = False
