...
async def test_workflow_task_decomposition():
    ...
    try:
        ...
        print(f"   {'✓' if funding_gap_mentioned else '✗'} 'Funding gap' mentioned: {funding_gap_mentioned}")
        ...
        simple_has_workflow = hasattr(simple_response, 'metadata') and \
                             simple_response.metadata and \
                             'workflow_id' in simple_response.metadata
        
        print(f"   {'✗' if simple_has_workflow else '✓'} Simple request bypassed workflow: {not simple_has_workflow}")
