...
class TestSingleAgentResponse(BaseE2ETest):
    ...
    async def test_1b1_response_consistency(self):
        ...
        # Print header
        formatter.print_test_header(
            test_name="test_1b1_response_consistency", description="Test agent response consistency"
        )
        ...
        try:
            ...
            for i, query in enumerate(queries, 1):
                ...
                response = await asyncio.wait_for(
                    overlord.chat(query, stream=False, user_id="test_user"), timeout=timeout
                )
                assert response is not None
                response_text = response.content if hasattr(response, "content") else str(response)
                assert len(response_text) > 0
            ...
            assert (
                unique_responses >= len(responses) // 2
            ), f"Only {unique_responses}/{len(responses)} unique responses"
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    result1 = asyncio.run(test.test_1b1_single_agent_response())
    print("\n" + "=" * 60)
