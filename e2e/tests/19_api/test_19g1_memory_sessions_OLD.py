...
They are NOT persisted, so session endpoints only show active sessions.
Tests focus on buffer memory operations instead.
"""
...
class TestMemorySessions(BaseE2ETest):
    ...
    def __init__(self):
        super().__init__(
            ...
            test_description="Test memory buffer operations (sessions are ephemeral)",
            test_area="19_api",
        )
    ...
    async def test_19g1_memory_sessions(self):
        ...
        try:
            ...
            assert data["object"] == "memory"  # API returns 'memory' for buffer memory
            ...
            # Test 4: Get sessions for user (sessions are ephemeral - only active ones shown)
            ...
            # Sessions are ephemeral - after chat completes, session is gone
            # So count will be 0 (no active sessions)
            print(f"   Session count: {session_count} (sessions are ephemeral)")
