...
- **Workflow tasks use LLM training-data date instead of system date** -- Agents executing SOP workflow steps consistently computed "today" as 2025-01-10 (Claude Sonnet's approximate training cutoff), regardless of the actual system date. No date context was injected into the task prompt, so the model used its internal prior. Fix: `_create_task_prompt` now includes `Current date/time: <system datetime>` in the Task Details block so all workflow tasks operate on the real date.
