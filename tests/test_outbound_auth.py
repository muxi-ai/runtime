...
from runtime.muxi.runtime.agent import Agent  # noqa: E402
    ...
    for test_case in test_cases:
        ...
        try:
            ...
            if success and test_case["should_succeed"]:
                print("   ✅ SUCCESS: Authentication worked as expected")
            elif not success and not test_case["should_succeed"]:
                print(
                    f"   ✅ SUCCESS: Failed as expected - {response.get('error', 'Unknown error')}"
                )
            elif success and not test_case["should_succeed"]:
                print("   ❌ UNEXPECTED: Should have failed but succeeded")
                print(f"      Response: {response}")
            else:
                print("   ❌ UNEXPECTED: Should have succeeded but failed")
...
async def test_auth_manager_directly():
    ...
    try:
        auth_manager.add_credentials("test-agent", AuthType.API_KEY, {"api_key": "test-key-123"})
    ...
    if success and "X-API-Key" in updated_headers:
        print("✅ Authentication applied successfully")
