...
async def test_memory_initialization():
    ...
    formation_config = {
        ...
        "llm": {
            "models": [
                {"text": "openai/gpt-4o-mini", "settings": {"temperature": 0.7}},
                {"embedding": "openai/text-embedding-3-small", "settings": {"temperature": 0.0}},
            ],
    ...
    with patch("src.muxi.overlord.LLM") as mock_llm_class:
        ...
        if overlord.buffer_memory:
            ...
            assert overlord.buffer_memory.working_multiplier == 8
