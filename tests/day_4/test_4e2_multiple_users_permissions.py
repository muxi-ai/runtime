    ...
    try:
        ...
        def run_test():
            async def test_operations():
                ...
                if any(term in response1_lower for term in 
                      ["credential", "token", "authenticate", "github mcp"]):
                ...
                assert not any(term in response2_lower for term in 
                              ["secret123", "api_key=secret", "found private"]), \
                    "User2 should not see User1's private content"
                
                # Should either ask for credentials or show no results
                assert any(term in response2_lower for term in 
                          ["credential", "no gists", "none", "empty", "authenticate", 
                           "cannot", "unable"]), \
                ...
                assert "user1" not in response2_lower and \
                       not any(term in response1_lower for term in response2_lower.split() 
                              if len(term) > 10), \
                ...
                assert not any(term in response.lower() for term in 
                              ["piepilot", "user1", "secret", "private"]) or \
                       any(term in response.lower() for term in 
                           ["credential", "authenticate", "no access"]), \
                ...
                assert not any(term in response_lower for term in 
                              ["yes", "can see", "will see", "access others"]), \
