        ...
        def run_test():
            async def test_operations():
                ...
                response = await overlord.chat(
                    "Show me my recent GitHub gists", user_id="user1", use_async=False
                )
                ...
                if any(
                    term in response_lower
                    for term in ["credential", "token", "authentication", "provide"]
                ):
                ...
                assert (
                    any(
                        term in response_lower
                        for term in ["gist", "github", "no gists", "empty", "none", "list"]
                    )
                    or len(response) > 50
                ...
                assert (
                    "gist" in response_lower
                    or "github" in response_lower
                ...
                response = await overlord.chat(
                    "How many GitHub gists do I have?", user_id="user1", use_async=False
                )
                print(f"Response: {response}")
                ...
                assert any(
                    term in response_lower
                    for term in ["gist", "number", "count", "total", "have", "none", "zero"]
                ) or any(char.isdigit() for char in response), "Response should provide gist count"
                ...
                response = await overlord.chat(
                    "List my public GitHub gists only", user_id="user1", use_async=False
                )
                print(f"Response: {response}")
