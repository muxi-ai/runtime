    ...
    try:
        ...
        def run_test():
            async def test_operations():
                ...
                assert any(term in response_lower for term in ["file", "created", "saved", "wrote"]), \
                ...
                assert any(term in content.lower() for term in ["memory", "ram", "usage"]), \
                ...
                assert any(term in report_content for term in ["memory", "ram"]), \
                    "Report should contain memory information"
                assert any(term in report_content for term in ["disk", "storage"]), \
                    "Report should contain disk information"
                assert any(term in report_content for term in ["uptime", "running"]), \
                ...
                try:
                    ...
                    assert "cpu" in content.lower() and "memory" in content.lower(), \
