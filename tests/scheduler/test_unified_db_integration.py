...
def test_formation_creates_shared_database_manager():
    ...
    with patch('muxi.services.db.get_database_manager') as mock_get_db:
...
def test_long_term_memory_uses_provided_db_manager():
    ...
    with patch('muxi.services.memory.long_term.Base'):
        with patch('muxi.services.memory.long_term.observability'):
...
def test_scheduler_service_uses_overlord_db_manager():
    ...
    with patch('muxi.services.scheduler.service.get_database_manager') as mock_get_db:
        with patch('muxi.services.scheduler.service.JobManager'):
            with patch('muxi.services.scheduler.service.ScheduleParser'):
                with patch('muxi.services.scheduler.service.PromptRewriter'):
