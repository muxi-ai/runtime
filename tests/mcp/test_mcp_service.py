...
from src.muxi.services.mcp.parser import ToolCall, ToolParser
...
class TestMCPService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.handler_patcher = patch(
            "muxi.services.mcp.handler.MCPHandler", return_value=self.mock_handler
        )
...
class TestAgentToolIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mcp_service_patcher = patch(
            "muxi.services.mcp.service.MCPService.get_instance", return_value=self.mock_mcp_service
        )
        self.mcp_service_patcher.start()

        # Import the Agent class after patching
        from muxi import Agent
        from muxi.formation.mcpserver import MCPServer
    ...
    @patch("muxi.services.mcp.parser.ToolParser.parse")
    @patch("muxi.agent.Agent.invoke_tool")
    @async_test
