...
class TestPhase6ComprehensiveSchemaValidation:
        ...
        complex_formation = {
            ...
            'a2a': {
                ...
                'inbound': {
                    'enabled': True,
                    'registries': [
                        'https://internal.phase6.test.com'
                    ],
                    'port': 8383,
                    'trusted_endpoints': [
                        'trusted.phase6.test.com',
                        'partner.phase6.test.com'
                    ],
                    'mode': 'api_key',
        ...
        boundary_formation = {
            ...
            'overlord': {
                'config': {
                    ...
                    'max_tool_calls': 0,          # Zero (unlimited)
    ...
    def test_secrets_interpolation_comprehensive(self):
        ...
        secrets_formation = {
            ...
            'overlord': {
                'llm': {
                    'api_key': '${{ secrets.COMPREHENSIVE_OVERLORD_API_KEY }}'
                }
            },
            ...
            'a2a': {
                'inbound': {
                    'shared_key': '${{ secrets.COMPREHENSIVE_A2A_SHARED_KEY }}'
                }
            }
        }
    ...
    def test_unknown_fields_detection_comprehensive(self):
        ...
        unknown_fields_formation = {
                ...
                'models': [
                    {
                        ...
                        'settings': {
                            'unknown_model_setting': 'should warn'
                        }
                    }
                ]
            },
            ...
            'overlord': {
                ...
                'config': {
                    'unknown_overlord_config': 'should warn'
                }
            },
            ...
            'memory': {
                ...
                'buffer': {
                    'unknown_buffer_field': 'should warn'
                },
                'long_term': {
                    'unknown_long_term_field': 'should warn'
                }
            },

            # Logging unknown fields
            'logging': {
                'unknown_logging_field': 'should warn'
            },
            ...
            'a2a': {
                ...
                'outbound': {
                    'unknown_outbound_field': 'should warn'
                },
                'inbound': {
                    'unknown_inbound_field': 'should warn'
                }
            },

            # MCP unknown fields
            'mcp': {
                'unknown_mcp_field': 'should warn'
            }
        }
        ...
        assert len(result.warnings) >= 8, f"Should warn about multiple unknown fields, got {len(result.warnings)} warnings"
    ...
    def test_performance_with_maximum_complexity(self):
        ...
        assert validation_time < 10.0, f"Validation took too long: {validation_time:.3f}s (should be < 10s)"
    ...
    def test_mixed_authentication_scenarios(self):
        ...
        auth_scenarios = [
            # No authentication
            {
                'schema': '1.0.0',
                'id': 'no-auth',
                'description': 'Formation without authentication'
            },
            # Only admin key
            {
                'schema': '1.0.0',
                ...
                'auth': {
                    'api_keys': {
                        'admin_key': '${{ secrets.ADMIN_ONLY_KEY }}'
                    }
                }
            },
            # Only user key
            {
                ...
                'auth': {
                    'api_keys': {
                        'user_key': '${{ secrets.USER_ONLY_KEY }}'
                    }
                }
            },
            # Both keys
