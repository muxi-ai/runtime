...
from src.muxi.config.validation import ValidationResult, FormationValidator
...
class TestFormationIntegration:
    ...
    def sample_formation_config(self):
        """Sample formation configuration for testing with new schema format."""
        return {
            'schema': '1.0.0',
            'id': 'test-formation',
            'description': 'Test formation for integration testing',
            'system_message': ('You are an orchestrator responsible for routing '
                               'messages to appropriate agents.'),

            # LLM configuration with new capability-based structure
            'llm': {
                'settings': {
                    'temperature': 0.7,
                    'max_tokens': 1000,
            ...
            # New overlord configuration structure
            'overlord': {
    ...
    def temp_modular_formation(self, sample_formation_config):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            main_config = {
                'schema': sample_formation_config['schema'],
                'id': sample_formation_config['id'],
                'description': sample_formation_config['description'],
                'system_message': sample_formation_config['system_message'],
                'llm': sample_formation_config['llm'],
                'overlord': sample_formation_config['overlord']
            }
            ...
            for agent in sample_formation_config['agents']:
                agent_file = agents_dir / f"{agent['id']}.yaml"
    ...
    async def test_overlord_load_formation_from_path(self, mock_create_model, temp_formation_file):
        ...
        with patch.object(overlord, 'create_agent'), \
            ...
            assert formation_config['id'] == 'test-formation'
            assert formation_config['schema'] == '1.0.0'
