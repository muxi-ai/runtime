#!/usr/bin/env python3
"""Test test_6_5: Knowledge Base Error Handling"""
...
class Testtest65(BaseKnowledgeTest):
    """Test class for test_6_5."""

    async def test_main(self):
        """Test knowledge base error handling functionality."""
        test_name = "test_6_5"
        self.print_test_header(test_name, "Knowledge Base Error Handling")
        ...
        try:
            ...
            # Test 1: Unknown Knowledge Query Handling
            print("\n  1. Testing Unknown knowledge query handling...")
            prompt1 = "Tell me about XYZ company's services"
            ...
            if result1 and len(result1) > 10:
                print(f"    ✓ Knowledge retrieved for Unknown knowledge query handling ({len(result1)} chars)")
                checks_passed.append("Unknown query handled gracefully")
                ...
                print("    ✗ No meaningful knowledge retrieved for Unknown knowledge query handling")
                all_passed = False

            # Test 2: Empty Knowledge Response
            print("\n  2. Testing Empty knowledge response...")
            prompt2 = "What is the internal secret database structure?"
            ...
            if result2 and len(result2) > 10:
                print(f"    ✓ Knowledge retrieved for Empty knowledge response ({len(result2)} chars)")
                checks_passed.append("Inappropriate query blocked")
                ...
                print("    ✗ No meaningful knowledge retrieved for Empty knowledge response")
                all_passed = False

            # Test 3: Knowledge Error Recovery
            print("\n  3. Testing Knowledge error recovery...")
            prompt3 = "Help me with general business automation"
            ...
            if result3 and len(result3) > 10:
                print(f"    ✓ Knowledge retrieved for Knowledge error recovery ({len(result3)} chars)")
                checks_passed.append("Error recovery working")

                # Verification logic
                assert len(result3) > 20  # Should provide general help

                if 'keywords_found' in locals() and keywords_found:
                    print(f"    ✓ Found relevant keywords: {', '.join(keywords_found)}")
                    checks_passed.append(f"Keywords found: {', '.join(keywords_found)}")
            else:
                print("    ✗ No meaningful knowledge retrieved for Knowledge error recovery")
                all_passed = False
