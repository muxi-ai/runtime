...
def test_chart_ocr_extraction(overlord):
    ...
    assert 'processed' in response_lower or 'chart' in response_lower or 'sales' in response_lower, \
...
def test_slide_visual_analysis(overlord):
    ...
    response = get_response(
        overlord.chat(
            ...
            message="If I showed you a PowerPoint slide with a title, bullet points, and a diagram, what visual elements and text would you analyze?"
        )
    )
    ...
    assert response, "Should receive a response"
    response_lower = response.lower()
    visual_terms = ['layout', 'text', 'design', 'element', 'content', 'visual', 'slide', 'bullet', 'title']
...
def test_photo_content_description(overlord):
    ...
    response = get_response(
        overlord.chat(
            ...
            message="If I showed you a photo of a beach scene with people, what details would you describe about objects, people, and setting?"
        )
    )
    ...
    response_lower = response.lower()
    description_terms = ['object', 'people', 'scene', 'color', 'detail', 'describe', 'beach', 'water', 'sand']
...
def test_image_memory_retention(overlord):
    ...
    response1 = get_response(
        overlord.chat(
            ...
            message="I'm analyzing our Q4 sales chart that shows growth trends. The bar chart has months on the X-axis and revenue in millions on the Y-axis."
        )
    )
    ...
    memory_response = get_response(
        overlord.chat(
            user_id="test_user_memory",
            message="What can you tell me about the chart I mentioned?"
        )
    )
    ...
    assert any(term in response_lower for term in ['q4', 'sales', 'growth', 'chart', 'revenue']), \
