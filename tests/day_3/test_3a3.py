...
async def test_document_comparison():
    ...
    # Read test documents from test-docs
    doc1_path = Path(__file__).parent.parent.parent / "test-docs" / "report.pdf"
    doc2_path = Path(__file__).parent.parent.parent / "test-docs" / "document.docx"
...
async def test_mixed_media_analysis():
    ...
    # Read mixed media files from test-docs
    csv_path = Path(__file__).parent.parent.parent / "test-docs" / "spreadsheet.csv"
    xlsx_path = Path(__file__).parent.parent.parent / "test-docs" / "spreadsheet.xlsx"
    pdf_path = Path(__file__).parent.parent.parent / "test-docs" / "small.pdf"
...
async def test_large_document_set():
    ...
    # Read multiple PDFs from test-docs for large document set
    ...
    for pdf_file in pdf_files:
        pdf_path = Path(__file__).parent.parent.parent / "test-docs" / pdf_file
        with open(pdf_path, "rb") as f:
            content = f.read()
        files.append({
            "filename": pdf_file,
