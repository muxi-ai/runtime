...
def test_pdf_basic_processing(overlord):
    ...
    assert (
        "processed" in response_lower
        or "document" in response_lower
        or "sample_document.txt" in response_lower
    ), "Response should indicate document was processed"
...
def test_async_processing(overlord):
    ...
    from tests.day_3.test_utils import is_async_response
    
    if is_async_response(response1):
        print("✓ Received async processing response")
        assert "Processing async request" in response1, "Should indicate async processing"
        assert "webhook" in response1.lower(), "Should mention webhook"
    else:
