"""
Test simple 'hi' with forced async using new approach
"""
...
async def test_hi_force_async():
    """Simple 'hi' test with forced async and new webhook handling"""
    ...
    print("\n=== Simple 'hi' Test with Forced Async ===")
    
    # Send simple message with explicit use_async=True
    response = await overlord.chat(
        user_id="test_hi_async_user",
        message="hi",
    ...
    result, was_async = check_response_with_webhook(
        ...
        min_keywords=1,
        min_length=10,
        timeout=60,  # Give more time for async
        test_name="Hi Test (Forced Async)"
    )
    ...
    if was_async:
        print("  - ✓ Async processing worked as requested!")
    else:
        print("  - ⚠️  Note: Requested async but got sync response")
        print("       (This can happen if formation overrides or request is too simple)")
...
if __name__ == "__main__":
    ...
    was_async = asyncio.run(test_hi_force_async())
