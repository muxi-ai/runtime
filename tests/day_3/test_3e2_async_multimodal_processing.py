...
def test_async_video_processing_request(overlord):
    ...
    response = get_response(
        overlord.chat(
            ...
            message="Analyze a 10-hour conference video with multiple speakers, presentations, and panel discussions. Extract all key insights, create timestamps for important moments, identify all speakers, and summarize each session.",
            use_async=True
        )
    )
...
def test_async_decision_making(overlord):
    ...
    complex_response = get_response(
        overlord.chat(
            user_id="test_user_decision",
            message="Create a comprehensive 50-point checklist for analyzing enterprise-scale multimodal datasets including compliance, quality assurance, and performance optimization considerations.",
            use_async=None  # Let system decide
        )
    )
...
def test_async_with_webhook(overlord):
    ...
    try:
        ...
        vis.sending_message(
            message="Process this large multimodal analysis task: Compare 100 research papers with their associated datasets and create a comprehensive meta-analysis.",
        ...
        if isinstance(response, dict) and 'request_id' in response:
            ...
            vis.step("Webhook should be called soon", f"Monitor {webhook_url} for incoming POST request")
            
            # Give the background task time to execute
            vis.step("Waiting for background task to send webhook", "Waiting 5 seconds for webhook delivery")
