...
class MockOverlord:
    ...
    async def _process_sync_chat_simplified(self, message, agent_name, user_id, session_id, request_id):
        """Simplified version of _process_sync_chat for testing."""
        
        # Check if message contains a credential token
        contains_token = await self.clarification.looks_like_credential_token(message) if (session_id and self.clarification) else False
            ...
            if session_id in self._pending_clarifications:
                ...
                if clarification_info.get("type") == "credential":
                    ...
                    if self.credential_resolver:
                        extracted_token = await self.clarification.extract_token_from_text(message) if self.clarification else None
                        if extracted_token:
                            await self.credential_resolver.set_credential(
                                service=service,
                                credential=extracted_token,
                                user_id=user_id
                            )
