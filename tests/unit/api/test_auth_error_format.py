    ...
    async with httpx.AsyncClient() as client:
        ...
        test_cases = [
            ...
            },
            {
                ...
                "headers": {},
                "expected_error_code": "FORBIDDEN",
                "expected_message": "A valid client API key is required. Please provide the 'X-Muxi-Client-Key' header.",  # noqa: E501
            },
            {
        ...
        for test in test_cases:
            ...
            try:
                response = await client.get(
                    f"{BASE_URL}{test['endpoint']}", headers=test["headers"]
                )
                ...
                try:
                    ...
                    has_envelope = all(
                        key in body
                        for key in ["object", "timestamp", "type", "request", "success", "error"]
                    )
                    ...
                    if has_envelope:
                        ...
                        if body.get("error"):
                            ...
                            if error.get("code") == test["expected_error_code"]:
                                print(
                                    f"  ✅ Error code matches expected: {test['expected_error_code']}"
                                )
                            else:
                                print(
                                    f"  ❌ Error code mismatch! Expected: {test['expected_error_code']}, Got: {error.get('code')}"  # noqa: E501
                                )
