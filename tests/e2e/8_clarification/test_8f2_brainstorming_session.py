...
async def test_brainstorming_session():
    ...
    try:
        ...
        print(
            "\n**User:** I want to brainstorm ideas for a new mobile app. "
            "Ask me questions to help develop the concept."
        )
        ...
        # Signal completion
        print(
            "\n**User:** I think we have enough to work with. "
            "Let's create a PDF document that summarizes these ideas into a concept."
        )
        ...
        if (
            "Does this approach work for you" in response6.content
            or "Should I proceed" in response6.content
        ):
        ...
        if "gamification" in response4.content.lower() or "game" in response4.content.lower():
            if "challenge" in response5.content.lower() or "badge" in response5.content.lower():
                print(
                    "✓ System built upon previous answers (gamification → specific game features)"
                )
        ...
        includes_user_input = (
            "finance" in final_response
            and "gamification" in final_response
            and ("challenge" in final_response or "badge" in final_response)
        )
        ...
        # Overall assessment
        success_criteria = (
            probing_questions >= 3 and len(brainstorm_elements) >= 3 and has_synthesis
        )
