    ...
    try:
        ...
        try:
            response2 = await asyncio.wait_for(
                overlord.chat("I'm working on a Python project", user_id=ctx.user_id, session_id=ctx.session_id, stream=False),
                timeout=5.0
            )
        ...
        try:
            response3 = await asyncio.wait_for(
                overlord.chat("What testing framework would you recommend?", user_id=ctx.user_id, session_id=ctx.session_id, stream=False),
                timeout=5.0
            )
            ...
            mentions_python = any(term in content3.lower() for term in ["pytest", "unittest", "python"])
