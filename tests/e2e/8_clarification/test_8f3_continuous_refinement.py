...
async def test_continuous_refinement():
    ...
    try:
        ...
        print("\n**User:** I need help designing a database schema. Keep asking questions until we have a complete design.")
        ...
        all_responses = [response1.content, response2.content, response3.content, 
                        response4.content, response5.content, response6.content]
        ...
        if any(word in ' '.join(response_lower[1:3]) for word in ['scale', 'size', 'users', 'performance']):
            specificity_progression.append('scale requirements')
        
        # Later questions should be detailed
        if any(word in ' '.join(response_lower[3:]) for word in ['specific', 'detail', 'particular', 'how']):
        ...
        # Overall assessment
        success_criteria = (
            refinement_questions >= 4 and
            len(specificity_progression) >= 2 and
            len(context_building) >= 2 and
            len(schema_elements) >= 3
        )
