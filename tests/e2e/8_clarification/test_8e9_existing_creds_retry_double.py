#!/usr/bin/env python
"""Test user1 with existing credentials adding new account with retry (double failure)."""
...
async def test():
    ...
    try:
        ...
        ctx = TestContext("existing_creds_retry_double")
        user_id = "user1"  # Using user1 who has existing credentials
        
        print("\n" + "="*60)
        print("CHAT TRANSCRIPT - Existing Creds + Double Retry")
        print("="*60)
        ...
        # Step 5: Good token
        print("\n**User:** wait, found the right one: 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")
        response5 = await overlord.chat(
            message="wait, found the right one: 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",
            user_id=user_id,
            session_id=ctx.session_id,
            stream=False
        )
        ...
        if any(indicator in response5.content.lower() for indicator in success_indicators):
            print("\n✅ SUCCESS: Double retry mechanism worked with existing credentials")
            ...
            print(f"Final response: {response5.content[:200]}...")
