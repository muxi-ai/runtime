...
class TestFormationLoading:
    ...
    async def test_1a2_directory_structure_formation(self):
        ...
        response = await overlord.chat("Hello, how are you?", user_id="test_user")
    ...
    async def test_1a3_formation_validation_failures(self):
        ...
        with pytest.raises(Exception):  # Could be yaml.YAMLError or other
            await formation.load("test-formations/invalid-formations/invalid-not-yaml.txt")
        
        # Test 2: Invalid YAML syntax
        formation = Formation()
        with pytest.raises(Exception):  # yaml.YAMLError expected
            await formation.load("test-formations/invalid-formations/invalid-syntax.yaml")
        
        # Test 3: Missing required keys
        formation = Formation()
        with pytest.raises(ConfigurationValidationError) as exc_info:
            await formation.load("test-formations/invalid-formations/invalid-missing-keys.yaml")
        ...
        formation = Formation()
        with pytest.raises(ConfigurationValidationError) as exc_info:
            await formation.load("test-formations/invalid-formations/invalid-schema.yaml")
        ...
        formation = Formation()
        with pytest.raises(ConfigurationValidationError) as exc_info:
            await formation.load("test-formations/invalid-formations/invalid-values.yaml")
    ...
    async def test_1a3_additional_validation_cases(self):
        """Test 1A3 Additional: More validation edge cases"""
        
        # Test: Empty YAML file
        formation = Formation()
        with pytest.raises(ConfigurationValidationError):
            await formation.load("test-formations/invalid-formations/invalid-empty.yaml")
        ...
        with pytest.raises(Exception):  # Could be FileNotFoundError or ValidationError
            await formation.load("test-formations/invalid-formations/invalid-no-agents/")
    ...
    async def test_1a4_flattened_formation_loading(self):
        ...
        formation = Formation()
        await formation.load("test-formations/formation-basic/formation-flattened.yaml")
        assert formation is not None
        assert formation.formation_id == "basic-test-formation"
