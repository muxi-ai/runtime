...
async def test_local_buffer_memory():
    ...
    def run_test():
        ...
        try:
            ...
            # Add context
            response1 = asyncio.run(overlord.chat("My name is Alice and I work at TechCorp", user_id="user1"))
            print(f"Context set: {response1[:50]}...")
            
            # Test recall
            response2 = asyncio.run(overlord.chat("What's my name and where do I work?", user_id="user1"))
...
async def test_remote_buffer_memory():
    ...
    def run_test():
        ...
        try:
            ...
            # Add context
            response1 = asyncio.run(overlord.chat("My name is Bob and I prefer JavaScript", user_id="user2"))
            print(f"Context set: {response1[:50]}...")
            
            # Test recall
            response2 = asyncio.run(overlord.chat("What's my name and what language do I prefer?", user_id="user2"))
            print(f"Context recall: {response2[:100]}...")
...
async def main():
    ...
    print(f"Local Buffer Mode: {'✅ PASS' if local_result.get('status') == 'success' else '❌ FAIL'}")
    ...
    print(f"Remote Buffer Mode: {'✅ PASS' if remote_result.get('status') == 'success' else '❌ FAIL'}")
    ...
    all_passed = (
        local_result.get("status") == "success" and
        remote_result.get("status") == "success" and
        switching_result.get("both_working")
    )
