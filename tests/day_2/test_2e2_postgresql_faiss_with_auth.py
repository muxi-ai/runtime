...
def test_remote_buffer_with_auth():
    ...
    def run_test():
        ...
        formation_path = "/Users/ran/Projects/muxi/code/runtime/test-formations/formation-memory/formation-postgres-and-faissx-with-auth.yaml"
        ...
        try:
            ...
            response1 = asyncio.run(overlord.chat("My name is Alice and I work at TechCorp as a senior engineer"))
            logger.info(f"Response 1: {response1}")
            
            response2 = asyncio.run(overlord.chat("I specialize in distributed systems and cloud architecture"))
            logger.info(f"Response 2: {response2}")
            
            # Test 2: Test memory recall
            logger.info("\n=== Test 2: Testing memory recall ===")
            response3 = asyncio.run(overlord.chat("What's my name and what do I do?"))
            ...
            response4 = asyncio.run(overlord.chat("What technical areas am I good at?"))
            ...
            for i in range(15):  # More than buffer size of 10
                ...
                asyncio.run(overlord.chat(msg))
                logger.info(f"Stored message {i}")
            
            # Test if old messages are still accessible via remote storage
            response5 = asyncio.run(overlord.chat("What was technical fact 0?"))
