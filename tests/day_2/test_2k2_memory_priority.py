...
async def test_memory_priority():
    ...
    await overlord.chat("I'm allergic to peanuts - this is very important!", user_id=test_user, use_async=False)
    await asyncio.sleep(3)

    # Other important facts
    await overlord.chat("I'm diabetic and need to monitor sugar intake", user_id=test_user, use_async=False)
    await asyncio.sleep(3)
    ...
    response = await overlord.chat("Do I have any dietary restrictions or health concerns?", user_id=test_user
    , use_async=False)
    ...
    response = await overlord.chat("Can I eat this peanut butter sandwich?", user_id=test_user
    , use_async=False)
    ...
    assert "no" in response_text.lower() or "allerg" in response_text.lower() or "avoid" in response_text.lower(), \
    ...
    await overlord.chat("My blood type is O-negative, important for emergencies", user_id=test_user, use_async=False)
    await asyncio.sleep(3)

    # Query should still include all critical info
    response = await overlord.chat("What critical medical information should a doctor know about me?", user_id=test_user
    , use_async=False)
