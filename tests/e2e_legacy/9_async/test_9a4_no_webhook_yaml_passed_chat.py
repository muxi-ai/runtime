...
async def main():
    ...
    print("🚀 MUXI Runtime - Test 9A4: No webhook in YAML, passed to chat()")
    print("="*60)

    formation_path = Path(__file__).parent / "formation-async" / "formation-no-webhook.yaml"
    ...
    try:
        ...
        print("\n✅ Formation loaded (no webhook in YAML)")
        print("📋 Testing async mode with webhook_url passed to chat()...")

        # Simple request but forced to async mode with webhook passed directly
        ...
        if isinstance(response, dict) and 'request_id' in response:
            ...
            if webhook_found:
                ...
                print("\n" + "="*60)
                print("❌ Test 9A4 FAILED: Webhook was not delivered")
                return False
            ...
            print("\n" + "="*60)
            print("❌ Test 9A4 FAILED: Did not get expected async processing response")
            return False
