...
async def test_api_key_redirect_mode():
    ...
    try:
        ...
        # Use actual user1 which has multiple credentials in the database
        user_id = "user1"  # This user has 2 GitHub credentials
        ...
        has_redirect_message = any(indicator in response_lower for indicator in redirect_indicators)

        if not has_redirect_message:
