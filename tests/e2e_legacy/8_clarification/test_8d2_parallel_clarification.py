...
async def test_8d2_parallel_source_clarification():
    ...
    try:
        ...
        assert any(
            word in response1.content.lower() for word in ["first", "source", "what", "which"]
        ...
        response2 = await asyncio.wait_for(
            overlord.chat(
                "PostgreSQL database", user_id=ctx.user_id, session_id=ctx.session_id, stream=False
            ),
            timeout=120.0,
        )
        ...
        assert any(
            word in response_lower for word in ["second", "other", "database", "table", "what"]
        ...
        response3 = await asyncio.wait_for(
            overlord.chat(
                "REST API endpoint", user_id=ctx.user_id, session_id=ctx.session_id, stream=False
            ),
            timeout=120.0,
        )
        ...
        needs_details = any(
            ...
            for word in ["table", "endpoint", "url", "which", "compare", "field", "column"]
        )
        ...
        assert any(
            word in response_lower for word in ["api", "endpoint", "url", "rest"]
        ...
        has_comparison_context = any(
            word in response_lower for word in ["compare", "comparison", "match", "difference"]
        )

        # Either it lacks tools (expected) or provides comparison guidance
        assert (
            lacks_tools or has_comparison_context
        ...
        context_preserved = (
            ("postgresql" in response_lower or "database" in response_lower)
            and ("users" in response_lower)
            and ("api" in response_lower or "rest" in response_lower)
            and ("customers" in response_lower or "/api/v2/customers" in response_lower)
        )
        ...
        print(
            f"System: {response1.content[:300] + '...' if len(response1.content) > 300 else response1.content}"
        )
        print("\nUser: PostgreSQL database")
        print(
            f"System: {response2.content[:300] + '...' if len(response2.content) > 300 else response2.content}"
        )
        print("\nUser: REST API endpoint")
        print(
            f"System: {response3.content[:300] + '...' if len(response3.content) > 300 else response3.content}"
        )
        print("\nUser: The users table for the database")
        print(
            f"System: {response4.content[:300] + '...' if len(response4.content) > 300 else response4.content}"
        )
        print("\nUser: /api/v2/customers endpoint")
        print(
            f"System: {response5.content[:300] + '...' if len(response5.content) > 300 else response5.content}"
        )
        print("\nUser: What sources am I comparing?")
        print(
            f"System: {response6.content[:300] + '...' if len(response6.content) > 300 else response6.content}"
        )
        print("\n" + "=" * 40)
...
async def test_8d2_interleaved_clarifications():
    ...
    try:
        ...
        assert any(
            ...
            for word in ["metric", "performance", "which", "what", "period", "time"]
        ), "Should ask about metrics or time period"
        ...
        response_lower = response2.content.lower()
        assert any(
            word in response_lower for word in ["period", "time", "when", "team", "who", "send"]
        ), "Should ask about time period or team"
        ...
        response3 = await asyncio.wait_for(
            overlord.chat(
                "Last 30 days", user_id=ctx.user_id, session_id=ctx.session_id, stream=False
            ),
            timeout=120.0,
        )
        print(f"Response: {response3.content}")
        ...
        response_lower = response3.content.lower()
        assert any(
            word in response_lower for word in ["team", "who", "send", "email", "format", "report"]
        ...
        response_lower = response4.content.lower()
        has_all_context = (
            any(word in response_lower for word in ["api", "response", "error"])
            and any(word in response_lower for word in ["30", "days", "month"])
            and any(word in response_lower for word in ["engineering", "team", "email"])
        )
