...
async def test_5b1_word_document_creation():
    ...
    try:
        ...
        prompt = "Create a Word document with a project status report including sections for overview, progress, and next steps"  # noqa: E501
                    ...
                    "artifacts": [
                        {
                            ...
                            "has_data_url": bool(
                                artifact.data_url and artifact.data_url.startswith("data:")
                            ),
                            "is_word_doc": (
                                artifact.filename.endswith(".docx") if artifact.filename else False
                            ),
                        }
                    ...
                    "artifacts": [
                        {
                            ...
                            "has_thumbnail": bool(
                                artifact.thumbnail and artifact.thumbnail.startswith("data:")
                            ),
                            "is_pdf": (
                                artifact.filename.endswith(".pdf") if artifact.filename else False
                            ),
                        }
                        for artifact in response.artifacts
                    ],
...
async def test_5b3_multi_section_documents():
    ...
    try:
        ...
        if response.artifacts:
            ...
            if artifact.filename and (
                "proposal" in artifact.filename.lower() or artifact.filename.endswith(".docx")
            ):
            ...
            if any(
                ...
                for keyword in ["cover", "executive", "summary", "appendix", "section"]
            ):
