...
async def test_error_resilience():
    ...
    await overlord.chat("I like hiking in the mountains", user_id=test_user, use_async=False)
    await overlord.chat("My favorite food is pizza", user_id=test_user, use_async=False)
    ...
    if hasattr(overlord.buffer_memory_manager, "search_buffer_memory"):
        ...
        response = await overlord.chat("What do I like to do?", user_id=test_user, use_async=False)
        response_text = await get_response_text(response)
    ...
    if overlord.long_term_memory:
        ...
        response = await overlord.chat("What's my name?", user_id=test_user, use_async=False)
        response_text = await get_response_text(response)
    ...
    response = await overlord.chat("Hello, how are you?", user_id=test_user, use_async=False)
    response_text = await get_response_text(response)
    assert len(response_text) > 0, "System not operational after errors"

    # Can still store and retrieve
    await overlord.chat("Final test message", user_id=test_user, use_async=False)
