...
### New Files (4)
1. **`migrations/add_user_identifiers.sql`** (~60 lines)
   - PostgreSQL migration script
   
2. **`migrations/add_user_identifiers_sqlite.sql`** (~85 lines)
   - SQLite migration script with table rebuild
   
3. **`src/muxi/utils/user_resolution.py`** (~350 lines)
   - `resolve_user_identifier()` function
   - `associate_user_identifiers()` function
   - KV caching integration
   
4. **`e2e/tests/17_multiple_identities/`** (2 files)
   - `test_multi_identity_basic.py` (~150 lines)
   - `formation.yaml` (~20 lines)

### Modified Files (8)
1. **`migrations/init_schema.sql`** (+20 lines)
   - Added user_identifiers table
   - Removed external_user_id column
   
2. **`migrations/init_schema_sqlite.sql`** (+20 lines)
   - Added user_identifiers table
   - Removed external_user_id column
...
### Migration Testing
```bash
# Tested on PostgreSQL test database
psql postgresql://muxi@localhost/muxi_test < migrations/add_user_identifiers.sql

# Results:
# ✅ 9 users migrated successfully
# ✅ 9 identifiers created
# ✅ external_user_id column dropped
...
2. **Run Migration**
   ```bash
   psql -U muxi -d muxi_production < migrations/add_user_identifiers.sql
   ```
